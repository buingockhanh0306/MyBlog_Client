...
const AuthPage = () => {
  ...
  const [admins, setAdmins] = useState<IAuthProps[]>([
    { username: '', updatedAt: '' }
  ]);
  const { formValue, setFormValue, errors, validation } = useFormLogin({
    username: '',
    password: ''
  });
    ...
    <>
      ...
      {!loading
        ...
        <Box padding={'20px 20px'} color={'primaryColor'}>
          ...
          <AuthList auths={admins} onDelete={handleDelete} />
        ...
        : (
        <AuthIndexSkeleton numberRecord={4} />
          )}
    </>
  );
};
