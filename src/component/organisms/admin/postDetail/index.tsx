...
import {ICategory, IPost} from "@src/component/organisms/admin/createPostForm";
...
const PostDetail:React.FC<IPostDetail> = ({post, onSubmit, isEdit, onClickExit, categories}) => {
    ...
    return !isEdit ? (
        <Box>
            <Box mb={'30px'}>
                ...
                <Text fontSize={'lg'} ml={'20px'} color={'textColor'}>{post?.title}</Text>
            </Box>

            <Box mb={'30px'}>
                <HeadingChakra text={t('slug')} fontSize={'xl'}/>
                <Text fontSize={'lg'} ml={'20px'} color={'textColor'}>{post?.slug}</Text>
            </Box>
            <Box mb={'30px'}>
            ...
            <Box mb={'30px'}>
                ...
                <Text textAlign={'justify'} fontSize={'lg'} ml={'20px'} color={'textColor'}>{post?.description}</Text>
            </Box>

            <Box mb={'30px'}>
                <HeadingChakra text={t('thumbnailImage')} fontSize={'xl'}/>
                <Box
                    boxSize='sm' w={{md: '30%', base: '86%'}}
                    h={'100%'}
                    ml={'20px'}
                >
                    <ImageBorderZoom src={post.thumbnail ? imageURL + post.thumbnail : '/images/noImage.jpg'}/>
                </Box>
            </Box>
            ...
            <Box mb={'30px'}>
                ...
                <Box dangerouslySetInnerHTML={{ __html: post?.content }} textAlign={'justify'}/>
            </Box>
        </Box>
    ):
    <Box>
        ...
        <Box mb={'30px'}>
            <HeadingChakra text={t('slug')} fontSize={'xl'}/>
            <Text fontSize={'lg'} ml={'20px'} color={'textColor'}>{formValue?.slug}</Text>
        </Box>
        ...
        >
            ...
            <ButtonSolid size={'md'} text={t('update')} onClick={()=>handleUpdate()}/>
        </Flex>}
    </Box>
};
