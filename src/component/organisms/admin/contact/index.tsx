...
import {Box, Button, Flex, Image, Menu, MenuButton, MenuItemOption, MenuList, MenuOptionGroup} from "@chakra-ui/react";
...
const Contact: React.FC<IContactProps> = ({data, onSubmit}) => {
    ...
    const [formContact, setFormContact] = useState<contactProps>({icon: '', name: '', url: '', slug: ''})
    ...
    return (
        <Flex mt={'-20px'} gap={{md: 4, base: 0}} alignItems={'center'} justifyContent={'space-around'} flexDirection={{md: 'row', base:'column'}}>
            <Menu >
                <MenuButton
                    ml={'30px'}
                    mt={'20px'}
                ...
                <MenuList>
                    <MenuOptionGroup defaultValue={''} type='radio' onChange={(value)=>handleChangeContact(String(value))}>
                        {
                            data?.map(item=>(
            ...
            <Box w={'100%'}>
                <InputText value={formContact.url} placeholder={t('urlPlaceholder')} onChange={(e)=>handleChangeUrl(e.currentTarget.value)}/>
            </Box>
            <Box mt={'20px'}>
                <ButtonSolid size={'md'} text={t('add')} onClick={()=>handleAddContact()}/>
            </Box>
        </Flex>
    );
};
