...
const FormLogin: React.FC<IFormLoginProps> = ({onSubmit, msgError}) => {
    const { formValue, setFormValue, errors, setErrors, validation } = useFormLogin({ username: '', password: '' });
    ...
    return (
        ...
        >
            <Flex
                w={{md: '30%', base: '100%'}}
                h={{md: 'auto', base: '100%'}}
                bgColor={{md: 'white', base: 'linear(to-r, gray.300, yellow.400, pink.200)'}}
                borderRadius={'10px'}
                padding={'40px 30px'}
                boxShadow={'2xl'}
            ...
            >
                ...
                <InputText
                    ...
                    onChange={(e: React.FormEvent<HTMLInputElement>)=>handleChangeUsername(e)}
                />
                <InputPassword
