...
const LayoutAdmin = ({children}) => {
    const { t, i18n } = useTranslation('layoutLocale')
    const { isOpen, onOpen, onClose } = useDisclosure()
    const headerHeight = '60px';
    const router = useRouter()

    function findPath(path){
        return router.pathname.indexOf(path)===0 ? true : false
    }
    ...
    const sidebarItem = [
        {text: t('categoriesManager'), icon: <FaElementor/>, path: '/admin/categories'},
